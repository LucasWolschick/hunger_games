...
fn main() {
    ...
    let values = item_db.get_values("Value").unwrap().iter().map(|v| if let item_db::Element::Full(i) = v { i.parse().unwrap() } else { 0i64 }).collect::<Vec<_>>();
    let costs = item_db.get_values("Weight").unwrap().iter().map(|v| if let item_db::Element::Full(i) = v { i.parse().unwrap() } else { 0i64 }).collect::<Vec<_>>();

    let scores = |vars: Vars| -> i64 {
        mul(vars, values.as_slice())
    };
    
    let weight = |vars: Vars| -> i64 {
        mul(vars, costs.as_slice())
    };
    ...
    for path in best {
        ...
        let items: Vec<_> = items.iter().cloned().filter(|i| (path.vars >> ((i.id-1) as u64)) & 1 == 1).collect();
